...
def naive_bayes(training_data, test_data):
    class_data, priors, likelihoods = {}, {}, {}
    total_samples = len(training_data)

    for features, label in training_data:
        class_data.setdefault(label, []).append(features)

    for label, samples in class_data.items():
        priors[label] = len(samples) / total_samples
        feature_counts = Counter(f for sample in samples for f in sample)
        total_features = sum(feature_counts.values())
        likelihoods[label] = {f: (feature_counts[f] + 1) / (total_features + len(feature_counts)) for f in feature_counts}

    posteriors = {label: math.log(priors[label]) + sum(math.log(likelihoods[label].get(f, 1e-6)) for f in test_data) for label in priors}
    return max(posteriors, key=posteriors.get)

def accuracy(training_data, test_data):
    correct = sum(naive_bayes(training_data, features) == label for features, label in test_data)
...
print("Disease Prediction System\n-------------------------")
symptoms = ["fever", "cough", "fatigue", "sneezing", "runny_nose"]
user_symptoms = [s for s in symptoms if input(f"Do you have {s}? ").strip().lower() == "yes"]

# Prediction
result = naive_bayes(training_data, user_symptoms)
print(f"\nPrediction: The individual is {'likely' if result == 'condition' else 'unlikely'} to have the disease.")

# Accuracy
print(f"\nModel Accuracy: {accuracy(training_data, test_data):.2f}%")
